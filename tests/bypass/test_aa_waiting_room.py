...
def test_stuck_queue_has_a_deadline(monkeypatch):
    ...
    with pytest.raises(b._WaitingRoomTimeoutError, match="waiting room"):
...
def test_queue_timeout_does_not_open_another_browser(monkeypatch):
    ...
    close = AsyncMock()
    get = AsyncMock(side_effect=b._WaitingRoomTimeoutError("Queue timed out"))
    ...
    assert b._run_bypass_in_current_process(URL, retry=10) == ""
